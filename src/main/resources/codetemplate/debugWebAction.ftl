...
public class ${testClassName} {
    ...
    public String ${method.methodName}(<#if method.methodParams?? && (method.methodParams?size>0)><#list method.methodParams as methodParam>String p_${methodParam}<#if methodParam_has_next>,</#if></#list></#if>) throws Exception {
          ...
          <#-- 基础action直接return -->
          return new ${method.className}(driver).excute(<#if method.methodParams?? && (method.methodParams?size>0)><#list method.methodParams as methodParam>p_${methodParam}<#if methodParam_has_next>,</#if></#list></#if>);
          <#-- 非基础action -->
         <#else>
